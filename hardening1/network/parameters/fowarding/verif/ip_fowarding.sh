#!/usr/bin/env bash
{
	...
	ipv6_chk()
	{
		...
		if grep -Pqs "^\h*net\.ipv6\.conf\.all\.disable_ipv6\h*=\h*1\h*(#.*)?$" $l_searchloc && grep -Pqs  "^\h*net\.ipv6\.conf\.default\.disable_ipv6\h*=\h*1\h*(#.*)?$" $l_searchloc && sysctl net.ipv6.conf.all.disable_ipv6 | grep -Pqs "^\h*net\.ipv6\.conf\.all\.disable_ipv6\h*=\h*1\h*(#.*)?$" && /sbin/sysctl net.ipv6.conf.default.disable_ipv6 | grep -Pqs "^\h*net\.ipv6\.conf\.default\.disable_ipv6\h*=\h*1\h*(#.*)?$"; then
			l_ipv6s="disabled"
		fi
		if [ -n "$l_ipv6s" ]; then
